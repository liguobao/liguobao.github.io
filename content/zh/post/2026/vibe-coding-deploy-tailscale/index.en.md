...
Who doesn't have a k8s cluster these days? Right.

If your cluster hasn't crashed at least once or twice a year, are you even serious?

And who doesn't spin up a machine here and a server there?

So I started tinkering with building cross-LAN/private-network connectivity.

Some time ago I manually set up WireGuard in the cloud and got several nodes running — it was basically usable.

But manually deploying this stuff is always a pain; if you misconfigure a node, debugging is a nightmare.

I had also seen Tailscale recently and ran the official demo using their hosted service — it worked fine, but the free tier only allows five devices, which isn't suitable for us.
...
So, first get a server — a lightweight cloud VM will do.

## Discussing the Plan with GPT

- I talked with GPT to confirm feasibility.

![Feasibility](/img/post/vibe-coding-deploy-tailscale/1.jiagou.png)
...
Got the idea — let's get to work.

## Vibe Coding DevOps: Getting Started

I decided to have GPT generate a Vibe Coding task and let Copilot run it on the server to see how it goes.
...
I bought the cloud VM and pointed the two domains at it — now let's proceed.
...
- Make sure to run Vibe Coding tasks on the target server; otherwise many steps might run locally instead of on the server.
- Docker images can be slow to pull in China — consider exporting them locally and importing them to the server.
- Authentik user creation may not complete automatically; have Vibe Coding generate the necessary scripts to perform the remaining steps.

There aren't many other pitfalls. Have fun!
